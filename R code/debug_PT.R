  ...
  QA_QC_drift_output_directory='/nas/cee-water/cjgleason/calval/Processed data/CU/Munged drifts/reprocessed_2023_05_10/'
  QA_QC_PT_output_directory='/nas/cee-water/cjgleason/calval/Processed data/CU/Munged PT/'
  matched_output_directory= '/nas/cee-water/cjgleason/calval/Processed data/CU/Matched_drifts/'
  drift_node_directory='/nas/cee-water/cjgleason/calval/Processed data/CU/Data frames/reprocessed_2023_05_10/node/'

  passname='fake swot pass ID'
  swot_time_UTC=as.numeric(as.POSIXct('2023-04-26 21:44:47'))#first a date, then a numeric so it doens't scale the difference 
  ...
  if(nrow(drift_pt_join_df)==0){
    write.csv('there are no appropriate drifts for this pass',paste0(matched_output_directory,passname,'processdate',
                                        str_replace_all(as.character(Sys.Date()),'\\-','_'),'.csv'))
  }else{
  write.csv(direct_match_drift,paste0(matched_output_directory,passname,'processdate',
                                      str_replace_all(as.character(Sys.Date()),'\\-','_'),'.csv'))
