...
plot_wse=ggplot(reach_pt_drift_df) +
  ...
  geom_point(aes(x=mean_pt_wse_m,y=wse_bar,col=as.factor(wse_start)))  +
  geom_line(aes(x=mean_pt_wse_m,y=mean_pt_wse_m),col='black')+
...
node_pt_wse=read.csv('Willamette/SWORD products/node/Willamette_node_PT_wse.csv')%>%
  select(-X)%>%
  mutate(datetime=as.POSIXct(pt_time_UTC))

node_drift_wse=read.csv('Willamette/SWORD products/node/Willamette_node_wses.csv')%>%
  select(-X)%>%
  mutate(datetime=as.POSIXct(time))

node_pt_drift_df=difference_inner_join(node_pt_wse,node_drift_wse,by='datetime',distance_col='time diff',max_dist=15*60)%>%
...
plot_wse2=ggplot(node_pt_drift_df) +
  ...
  geom_point(aes(x=mean_pt_wse_m,y=node_wse,col=as.factor(node_id)))  +
  geom_line(aes(x=mean_pt_wse_m,y=mean_pt_wse_m),col='black')+
  labs(x='PT node wse',y='drift node wse', title= 'drift and pt within same node and 30min')
...
node_pt_wse=read.csv('Willamette/SWORD products/node/Willamette_node_PT_wse.csv')%>%
  ...
  summarize(node_averaged_to_reach=mean(mean_pt_wse_m))%>%
  ungroup()%>%
...
joined_reach_pt_df=left_join(node_pt_wse,reach_pt_wse,by=c('datetime','reach_id'))%>%
  transmute(pt_time_UTC=datetime,node_averaged_to_reach=node_averaged_to_reach, pt_averaged_in_reach=mean_pt_wse_m)
