...
#scale max width (in cases of under/overestimated)
scale_maxwidth = 2
...
for (eachnode in this_river_node_IDs) {
  ...
  waterarea = sum(r_binary_water[] == 1) * res(r_binary_water)[1] * res(r_binary_water)[2]
  ...
  waterarea_lower = sum(r_binary_water_lower[] == 1) * res(r_binary_water_lower)[1] * res(r_binary_water_lower)[2]
  ...
  waterarea_upper = sum(r_binary_water_upper[] == 1) * res(r_binary_water_upper)[1] * res(r_binary_water_upper)[2]
  ...
  Water_df = rbind(Water_df,data.frame(ReachID = reach_id_str,NodeID = node_id_str,Waterarea_m2 = waterarea, Waterareaerr_m2=waterarea_uncertainty,Date =strdate))
}
...
for (eachreach in this_river_reach_IDs) {
  ...
  # save variable values to data frame
  Water_df_reach = rbind(Water_df_reach,data.frame(ReachID = reach_id_str,Waterarea_m2 = waterarea, Waterareaerr_m2=waterarea_uncertainty,Date =strdate))
}
##save data frame to a csv file
