...
holdit=data.frame(matrix(nrow=1000,ncol=4))
names(holdit)=c('truemean','PTmean','truedelta','PTdelta')
for (i in 1:1000){
#sample 5 nodes
point1=8
...
holdit[i,]=  c(average_height_true,
  average_height_sample,
  true_delta,
  sample_delta)
...
stats_df=holdit%>%
  mutate(mean_dif=truemean-PTmean, delta_dif=truedelta-PTdelta)%>%
  select(mean_dif,delta_dif)%>%
  gather(Parameter,Difference_m)

p1=ggplot(stats_df)+
  geom_violin(aes(x=Parameter,y=Difference_m,fill=Parameter))

p2=ggplot(stats_df,aes(x=Parameter,y=Difference_m,fill=Parameter))+
  geom_boxplot()+
  geom_jitter(alpha=0.1)

p3=ggplot(filter(stats_df,Parameter=='delta_dif')%>%mutate(Difference_cm=Difference_m*100),
          aes(x=Parameter,y=Difference_cm,fill=Parameter))+
  geom_boxplot()+
  geom_jitter(alpha=0.1)


plot(cowplot::plot_grid(p1,p2,p3,nrow=2))