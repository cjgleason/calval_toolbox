...
lid <- catalog("D:/CTR Lidar 2021/Raw/CT_River_01002_filt34.las")
...
# Extract X and Y locations from PT file
x <- PT_loc$POINT_X
y <- PT_loc$POINT_Y
# Set radius
r <- 50 # meters # was 10, but does not capture some PT locations... 
# Save clipped las files 
name <- lid@data$File.Source.ID
opt_output_files(lid) <-  paste0("D:/CTR Lidar 2021/tmp/filt/", name, "_{XCENTER}_{YCENTER}")
...
# LAS_files <- list.files(path="D:/CTR Lidar 2021/tmp/filt", pattern="*.las", full.names=TRUE, recursive=FALSE)
# shp <- st_read("D:/CTR Lidar 2021/CT Sentinel-2 water mask/ctr_ndwi_buf10.shp")
# shp <- st_transform(shp, "+init=epsg:6347")
# 
...
PPP_files <- list.files(path = "D:/CTR Lidar 2021/GNSS/PT_install/", pattern = "*.csv", full.names=TRUE, recursive=FALSE)
...
for (i in PPP_files){
  PPP <- read.csv(i)
  PPP$name <- gsub(".*[/]([^.]+)[.].*", "\\1", i)
  PPP$date <- as.Date((PPP$day_of_year -1), origin=as.Date(paste0(as.character(PPP$year[1]),"-01-01")), format = "%Y-%m-%d")
  PPP$DT <- (with_tz(as.POSIXct(PPP$date), tz = "UTC") + (3600*PPP$decimal_hour))
  PPP_list[[i]] <- PPP[,c(1:3,7,9,11)]
}
...
for (i in 1:length(PT_grp)){
  for (k in 1:nrow(PT_grp[[i]])){
    ...
    avg = mean(PPP_filt$ortho_height_m_cgvd2013)
    id = PT_grp[[i]]$PT[k]
    gnss_offset = PT_grp[[i]]$GNSS_offset[k]
    DT_UTC = PT_grp[[i]]$roundtime[k]
    offset <- data.frame(avg, id, gnss_offset, DT_UTC)
...
### Get PT offset based on time and apply to each file ###
...
clipped_LAS_files <- list.files(path="D:/CTR Lidar 2021/tmp/filt", pattern="*_clip.las", full.names=TRUE, recursive=FALSE)
...
PT_loc$POINT_Y <- as.character(round(PT_loc$POINT_Y, digits = 0)) # Match y instead because of rounding and decimal
# Format as character with 2 digits for matching in loop #
PT_loc$PT <- sprintf("%02d", PT_loc$PT)

for (i in clipped_LAS_files) {
  las <- readLAS(i)
  lidid <- str_sub(i, start = -16, -10)
  PTid <- filter(PT_loc, POINT_Y ==lidid)$PT
...
avgtime <- merge(avgtime,PTdf[c("comb","PT_elev")],by = "comb")

### Add PT elev at time of lidar to lidar df ###
names(avgtime)[2] <- 'ID'
df<- merge(df, avgtime[c("ID", "PT_elev")], by = "ID")
...
sp <- ggplot(scatter, aes(x = PT_elev, y = pdfmax)) +
  geom_point(size = 2) +
  # geom_text_repel(label = scatter$PT_ID,
                  # size = 2.5) +
  ...
  xlab("PT Elev (m)") + ylab("Lidar Elev (m)") +
  xlim(30, 33.25) +
  ylim(30, 33.25) +
  theme_classic() +
  theme(# plot.title = element_text(size = 11, hjust = 0.5),
...
sp
png(file = paste0("D:/CTR Lidar 2021/figs/scatter.png"))
plot(sp)
dev.off()
  ...
  testplot <- 
    ggplot(df_grp[[i]], aes(x=Z, color = Z, fill = Z)) +
      ...
      theme(plot.title = element_text(size = 11, hjust = 0.5),
            ...
            panel.grid.major.y = element_line(color = "gray91", size = 0.25))
  # Save files
  png(file = paste0("D:/CTR Lidar 2021/figs/lidvPT_", df_grp[[i]]$ID[1], ".png"))
  plot(testplot)
  dev.off()
}
